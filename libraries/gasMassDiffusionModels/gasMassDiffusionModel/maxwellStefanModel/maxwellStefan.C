...
)
{
    ...
    //- Solving equation for internal surfaces
    forAll(mesh_.magSf(), faceI)
    {
        ...
        forAll(gasCompositionSpace_.gasSpecieTable(), i)
        {
                ...
                forAll(gasCompositionSpace_.gasSpecieTable(), j)
                {
                    if(i == j)
                    {
                        forAll(gasCompositionSpace_.gasSpecieTable(), k)
                        {
                            if(k != i)
                            {
                                A_(i,j) += volFrac_[k]()[faceI] / Dijf[i][k][faceI];
                            }
                        }
                    }
                    else
            ...
            else //- last row sum of flux equals zero
            {
                forAll(gasCompositionSpace_.gasSpecieTable(), j)
                {
                    A_(i,j) = 1.0 / Dijf[i][i][faceI];
                }
                b_[i] = 0.0;
            }

            //- convert matrix to diagranally 1 matrix
            scalar aii = A_(i,i);
            forAll(gasCompositionSpace_.gasSpecieTable(), j)
            {
                A_(i,j) /= aii;
    ...
    //- Solving equation for boundary surfaces
    forAll(gradMolConcentration_[0]().boundaryField(), patchI)
    {

        if(mesh_.boundaryMesh().types()[patchI] != "empty")
        {
